...
class AtomicVelocityView extends SelectListView {
    initialize(state) {
        ...
        this.docQuery.on("ready", () => {
            this.documentsLoaded = true;
            this.setLoading();
            return this.populateList();
        });
        this.docQuery.on("added", (fileDetails) => {
            if (this.documentsLoaded) {
                return this.populateList();
            }
        });
        this.docQuery.on("updated", (fileDetails) => {
            if (this.documentsLoaded) {
                return this.populateList();
            }
        });
        this.docQuery.on("removed", (fileDetails) => {
            if (this.documentsLoaded) {
                return this.populateList();
            }
        });
    ...
    confirmSelection() {
        ...
        sanitizedQuery = this.getFilterQuery().trim();
