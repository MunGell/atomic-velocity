...
export default class Utility {
    ...
    static getNoteDirectory() {
        return fs.normalize(atom.config.get('nvatom.directory'));
    }

    static getPrimaryNoteExtension() {
        if (atom.config.get('nvatom.extensions').length) {
            return atom.config.get('nvatom.extensions')[0];
        } else {
            return '.md';
        }
    }
    ...
    static isNote(filePath) {
        var ref;
        if (ref = path.extname(filePath), indexOf.call(atom.config.get('nvatom.extensions'), ref) < 0) {
            return false;
        }
